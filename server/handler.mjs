...
/**
 * The visitor's IP, not CloudFront's.
 *
 * `requestContext.http.sourceIp` is whoever opened the TCP connection —
 * and since OAC makes CloudFront the only possible caller, that is always
 * an edge POP. Keying limits off it would lump everyone behind one POP
 * into a single bucket (one busy city exhausts the quota for the whole
 * region) while an abuser rotating POPs collects a fresh quota per POP.
 *
 * CloudFront appends the real viewer IP to X-Forwarded-For, so the LAST
 * entry is the one it observed; earlier entries may be viewer-supplied
 * and are ignored. Unreachable except through CloudFront, so this is
 * trustworthy here in a way it would not be on a public origin.
 */
