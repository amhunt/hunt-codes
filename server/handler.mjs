 ...
 * any viewer-supplied copy, so it IS trustworthy. The `/api/*` behaviors
 * use the custom `hunt-codes-api-viewer-geo` origin request policy, which
 * forwards it (plus the CloudFront-Viewer-* geo headers below). If the
 * policy ever regresses to a managed one that drops it, this falls back
 * to `sourceIp`, which behind OAC is the edge POP: coarser than
 * per-visitor, but not forgeable. See server/README.md.
 */
const viewerIp = (event) => {
  const address = event.headers?.["cloudfront-viewer-address"];
  if (address) {
    // CloudFront always appends the source port: "198.51.100.10:46532",
    // and for IPv6 the bare form "2603:7000::bbac:57067" (no brackets —
    // verified against the live stack). Strip the trailing :port either
    // way; a bracketed "[::1]:46532" is handled too, in case that changes.
...
/**
 * Where the drawing was made, from CloudFront's viewer-location headers
 * (forwarded by the same origin request policy). Stored on each drawing
 * so they can be plotted on a globe later. Best-effort: CloudFront omits
 * headers it can't resolve, so every field is optional.
 */
// Origin request policies cap out at 10 headers, so the human-readable
// country/region *names* are left out — codes are enough to plot.
const GEO_HEADERS = {
  country: "cloudfront-viewer-country", // ISO 3166-1, e.g. "US"
  region: "cloudfront-viewer-country-region", // e.g. "NY"
  city: "cloudfront-viewer-city",
  timeZone: "cloudfront-viewer-time-zone",
};
const viewerGeo = (event) => {
  const headers = event.headers || {};
  const geo = {};
  for (const [key, header] of Object.entries(GEO_HEADERS)) {
    const value = headers[header];
    if (value) geo[key] = decodeURIComponent(value); // city names arrive URL-encoded
  }
...
// DynamoDB map for the geo object: strings as S, lat/lon as N
const geoToItem = (geo) => ({
    ...
    await ddb.send(
      new PutItemCommand({
        ...
        Item: {
          ...
          ipHash: { S: ipHash },
          // Raw viewer IP + CloudFront geolocation, kept for a future
          // "drawings on a globe" view. Never returned by the API
          // (drawingToJson picks its fields explicitly).
          ip: { S: ip },
          geo: geoToItem(geo),
          gsi1pk: { S: "drawings" },
